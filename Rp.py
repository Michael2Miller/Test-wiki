...
def _(key, lang_code):
    ...
    return LANGUAGES.get(lang_code, LANGUAGES[DEFAULT_DEFAULT_LANG]).get(key, LANGUAGES[DEFAULT_LANG].get(key, 'MISSING TRANSLATION'))
...
# --- NEW FUNCTION: Show Search Options ---
async def show_search_options(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_user_waiting_db(user_id):
        await update.message.reply_text(_('already_searching', lang_code), reply_markup=await get_keyboard(lang_code), protect_content=True)
        return
...
# --- NEW CALLBACK HANDLER: Handle Language Selection (Phase 2) ---
...
async def relay_and_log_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if LOG_CHANNEL_ID and sender_id != ADMIN_ID:
        ...
        try:
            ...
            sender_username = sender_user.username if sender_user.username else 'غير متاح'
            sender_name = sender_user.first_name
            sender_link_html = f'<a href="tg://user?id={sender_id}">{sender_name}</a>'
            ...
            # تحديد محتوى التقرير (الرسالة الثانية)
            if partner_id:
                # محادثة نشطة: نحتاج ID الشريك
                # Note: We need to try/catch partner_user fetch in case the user has been deleted or blocked the bot globally.
                try:
                    partner_user = await context.bot.get_chat_member(chat_id=partner_id, user_id=partner_id).user
                    partner_username = partner_user.username if partner_user.username else 'غير متاح'
                    partner_name = partner_user.first_name
                    
                    log_report = (
                        f"<b>الحالة:</b> محادثة نشطة\n"
                        f"👤 المرسل: {sender_link_html} (@{sender_username})\n"
                        f"<b>ID:</b> <code>{sender_id}</code>\n"
                        f"---<br>"
                        f"👈 المستقبِل: <a href='tg://user?id={partner_id}'>{partner_name}</a> (@{partner_username})\n"
                        f"<b>ID:</b> <code>{partner_id}</code>"
                    )
                except Exception:
                     # Fallback if partner info cannot be fetched
                    log_report = (
                        f"<b>الحالة:</b> محادثة نشطة (بيانات الشريك غير متاحة)\n"
                        f"👤 المرسل: {sender_link_html} (@{sender_username})\n"
                        f"<b>ID:</b> <code>{sender_id}</code>\n"
                        f"---<br>"
                        f"👈 المستقبِل: مجهول (ID: <code>{partner_id}</code>)"
                    )
            else:
                # رسالة منفردة: لا يوجد شريك
                log_report = (
                    f"<b>الحالة:</b> رسالة منفردة (لا يوجد شريك)\n"
                    f"👤 المرسل: {sender_link_html} (@{sender_username})\n"
                    f"<b>ID:</b> <code>{sender_id}</code>"
                )
            ...
            await context.bot.send_message(
                chat_id=LOG_CHANNEL_ID,
                text=log_report,
                parse_mode=constants.ParseMode.HTML, # نستخدم HTML لدعم التنسيق
                disable_notification=True,
...
def main():
    ...
    # تصحيح: توجيه زر البحث إلى دالة عرض الخيارات

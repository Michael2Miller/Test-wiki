...
async def relay_and_log_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- [مهم جداً] منع الأدمن من الدخول في هذا المستجيب إذا كان الأمر برودكاست ---
    # إذا كانت الرسالة من الأدمن وبدأت بـ /broadcast، لا تقم بمعالجتها كرسالة عادية
...
def main():
    ...
    # 2. مستجيب لبث الوسائط (صور/فيديو/ملفات) - تم تصحيح filters.DOCUMENT
    application.add_handler(MessageHandler(
        filters.User(user_id=ADMIN_ID) & 
        (filters.PHOTO | filters.VIDEO | filters.Document.ANY) & # <--- تم التصحيح النهائي هنا
        filters.CaptionRegex(r'^/broadcast'), # يتطلب وجود /broadcast في الكابشن
        broadcast_command
    ), group=1)

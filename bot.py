...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if raw_text:
        # إذا بدأت بـ /broadcast، قم بإزالة الأمر
        if raw_text.startswith("/broadcast"):
            message_to_send = raw_text[len("/broadcast"):].strip()
        # وإلا، استخدم النص كما هو (للسماح بإرسال أي رسالة أو كابشن كبث إذا كان الأدمن يرسلها)
        else:
    ...
    # التحقق من وجود محتوى للبث (صورة بدون كابشن تعتبر بثاً)
    if not is_media_broadcast and not message_to_send:
        # رسالة الاستخدام
        await message.reply_text(
            "Usage:\n"
            "1. For text: `/broadcast Your message here (use HTML for links/formatting)`\n"
            "2. For media: Send the photo/video/document with `/broadcast` and your message in the caption (use HTML).",
            protect_content=True
        )
        return
    ...
    # تحديد وضع التحليل لتمكين الروابط والتنسيقات المنسقة - تم التعديل إلى HTML

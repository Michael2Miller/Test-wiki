...
# --- (1) Translation Dictionaries and Helpers (تم إكمال جميع المفاتيح) ---
LANGUAGES = {
    'en': {
...
async def get_confirmation_keyboard(reported_id, lang_code):
    """(MODIFIED) لوحة تأكيد الحظر بناءً على اللغة."""
...
# --- (4) Subscription and Language Handlers (Moved up) ---
# (تم نقل هذه الدوال إلى هنا لحل خطأ NameError)
...
async def send_join_channel_message(update_or_query: Union[Update, Update.callback_query], context: ContextTypes.DEFAULT_TYPE, lang_code: str):
    ...
    keyboard = [
        [
            ...
            # (FIX) استخدام مفتاح ترجمة مناسب لزر الانضمام
    ...
    elif isinstance(update_or_query, Update.callback_query): 
...
async def sendid_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            parse_mode=None, # (تم التعديل) السماح بالروابط كنص عادي
            protect_content=True
        )
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # (الإصلاح: استخراج النص من التعليق أو الرسالة)
    message_to_send = None
    if message.caption:
        # إزالة /broadcast من التعليق
        if message.caption.startswith("/broadcast"):
            message_to_send = message.caption[len("/broadcast"):].strip()
        else:
             message_to_send = message.caption # (احتياطي)
    elif message.text:
        # إزالة /broadcast من النص
    ...
    for target_user_id in all_users:
        try:
            if is_media_broadcast and message.photo:
                await context.bot.send_photo(chat_id=target_user_id, photo=message.photo[-1].file_id, caption=message_to_send, protect_content=True, parse_mode=None)
            elif is_media_broadcast and message.video:
                await context.bot.send_video(chat_id=target_user_id, video=message.video.file_id, caption=message_to_send, protect_content=True, parse_mode=None)
            elif is_media_broadcast and message.document:
                 await context.bot.send_document(chat_id=target_user_id, document=message.document.file_id, caption=message_to_send, protect_content=True, parse_mode=None)
            elif message_to_send: # Text-only broadcast
                await context.bot.send_message(
                    chat_id=target_user_id, 
                    text=message_to_send, 
                    parse_mode=None, # (الإصلاح: إزالة Markdown للسماح بالروابط)
                    protect_content=True
                ) 

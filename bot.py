...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التحقق من أن هناك شيئاً للإرسال
...
# --- (6) Main Bot Handlers (User Commands) (Omitted for brevity, assumed unchanged) ---
    ...
    async with db_pool.acquire() as connection:
        async with connection.transaction():
            ...
            if partner_id:
                ...
                await context.bot.send_message(chat_id=user_id, text=_('partner_found', lang_code), reply_markup=get_keyboard(lang_code), protect_content=True)
    ...
    async with db_pool.acquire() as connection:
        async with connection.transaction():
            ...
            if partner_id_new:
                ...
                await context.bot.send_message(chat_id=user_id, text=_('partner_found', lang_code), reply_markup=await get_keyboard(lang_code), protect_content=True)
...
async def relay_and_log_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # جلب لغة الشريك وإضافة البادئة للرسائل النصية
        partner_lang = await get_user_language(partner_id)
...
def main():
    ...
    application.add_handler(MessageHandler(
        filters.ChatType.PRIVATE & (filters.COMMAND | filters.CAPTION), 
        broadcast_command
    ), group=1)
    # -----------------------------------------------------
    ...
    # 4. معالجات الأزرار النصية (Reply Buttons) (الأولوية 4)
    ...
    # 5. المعالج الرئيسي للرسائل (الأولوية 5 - الأخير)

    ...
    try:
        async with db_pool.acquire() as connection:
            try:
                ...
                # هذا يحدث فقط إذا كان الجدول قديماً، نعود للإنجليزية
                return DEFAULT_LANG
...
def _(key, lang_code):
    ...
    # Fallback logic: Try requested language -> Try default English -> Try English key
...
async def init_database():
    ...
    try:
        ...
        async with db_pool.acquire() as connection:
            ...
            try:
                # محاولة التحقق أولاً لتجنب الأخطاء
                await connection.execute("SELECT language FROM all_users LIMIT 1")
                 ...
                 try:
                    # قد يكون الجدول موجوداً بدون العمود
                    ...
                    logger.info("Added 'language' column to all_users table.")
                     ...
                     # نمرر الخطأ إذا كان الجدول لا يزال غير موجود (سيتم إنشاؤه لاحقاً)
                     pass
            ...
            await connection.execute('''
                CREATE TABLE IF NOT EXISTS all_users (
                    user_id BIGINT PRIMARY KEY,
                    language VARCHAR(5) DEFAULT 'en' -- للتأكد من وجود العمود
                );
            ''')
            await connection.execute('''
...
async def add_user_to_all_list(user_id, lang_code=None):
    ...
    # نستخدم اللغة الافتراضية إذا لم يتم تمرير أي لغة
...
# --- (5) Subscription and Language Handlers ---
# (Handles language setting logic that affects start_command and messages)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # (NEW) Language/User Check
    user_in_db = await check_if_user_exists(user_id)
    
    if not user_in_db:
        # User is completely new: Show language selection first
        await show_initial_language_selection(update, context)
        return
        
    # User exists: Continue with saved language
...
async def handle_block_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("cancel_block"):

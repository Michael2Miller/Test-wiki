...
def main():
    ...
    # 4. مستجيب لبث المستندات (باستخدام الاسم الصحيح)
    # الاسم الصحيح في v20+ هو filters.Document.ALL (وفقاً للوثائق)
    application.add_handler(MessageHandler(
        admin_filter & filters.Document.ALL & caption_filter, # <--- تم التصحيح النهائي هنا
        broadcast_command
    ), group=1)

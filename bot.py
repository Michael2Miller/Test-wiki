...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- (تعديل منطق استخراج الرسالة لإصلاح مشكلة الكابشن) ---
    ...
    if raw_text:
        ...
        # وإلا، استخدم النص كما هو (وهذا يسمح بإرسال أي رسالة نصية أو كابشن كبث دون كتابة /broadcast، وهو المنطق الأقوى للأدمن)
        else:
    ...
    if not is_media_broadcast and not message_to_send:
        ...
        await message.reply_text(
            "Usage:\n"
            "1. For text: `/broadcast Your message here (use Markdown V2 for links/formatting)`\n"
            "2. For media: Send the photo/video/document with `/broadcast` and your message in the caption (use Markdown V2).",
            protect_content=True
        )
        return
    ...
    # تحديد وضع التحليل لتمكين الروابط والتنسيقات المنسقة
    parse_mode_to_use = constants.ParseMode.MARKDOWN_V2

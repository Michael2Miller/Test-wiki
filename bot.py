...
# --- Settings & Environment Variables (Remains the same) ---
try:
...
async def get_confirmation_keyboard(reported_id, lang_code):
    ...
    cancel_text = _('cancel_op_btn', lang_code) # (NEW) استخدام المفتاح الجديد
    
    keyboard = [
        [InlineKeyboardButton("✅ " + _('block_btn', lang_code), callback_data=f"confirm_block_{reported_id}_{lang_code}")],
        [InlineKeyboardButton(cancel_text, callback_data=f"cancel_block_{lang_code}")] # (MODIFIED)
    ]
...
async def handle_language_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("set_lang_"):
        ...
        try:
            ...
            # (MODIFIED) إضافة الجملة الإرشادية الجديدة
            if new_lang_code == 'ar':

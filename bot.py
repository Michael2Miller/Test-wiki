...
async def relay_and_log_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if LOG_CHANNEL_ID and sender_id != ADMIN_ID:
        try:
            # إرسال الرسالة الأصلية إلى قناة الأرشفة
            ...
            logger.error(f"Failed to forward message from {sender_id} to LOG_CHANNEL_ID: {e}")
...
def main():
    ...
    # المستجيب العام لأي رسالة ليست أمر وليست زر
    application.add_handler(MessageHandler(

...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if raw_text and raw_text.startswith("/broadcast"):
        message_to_send = raw_text[len("/broadcast"):].strip()
    elif raw_text:
        message_to_send = raw_text.strip()
    ...
    if not is_media_broadcast and not message_to_send:
        ...
        await message.reply_text(
            "Usage:\n"
            "1. For text: `/broadcast Your message here (HTML supported)`\n"
            "2. For media (Recommended): Send the photo/video/document with `/broadcast` in the caption. The image/video will be FORWARDED.",
            protect_content=True
        )
        return
    ...
    # وضع التحليل للبادئة النصية (HTML الأكثر توافقاً)
    parse_mode_to_use = constants.ParseMode.HTML
    
    for target_user_id in all_users:
        try:
            # 1. إرسال البادئة أولاً
            await context.bot.send_message(
            ...
            if is_media_broadcast:
                # 2. إعادة توجيه الرسالة الأصلية (التي تحتوي على الوسائط والكابشن)
                # ملاحظة: إعادة التوجيه تتجاهل الكابشن الذي أرسلته في الخطوة 1، ولكنها تحافظ على الكابشن الأصلي للوسائط
                await context.bot.forward_message(
                    chat_id=target_user_id,
                    from_chat_id=user_id, # مرجع للمستخدم الذي أرسل الأمر
                ...
                await context.bot.send_message(
                    chat_id=target_user_id, 
                    text=message_to_send, # نرسل رسالة الأدمن النصية بدون البادئة التي تم إرسالها بالفعل في الخطوة 1
                    parse_mode=parse_mode_to_use, 
                    protect_content=True
                ) 

...
LANGUAGES = {
    ...
    'es': {
        ...
        'already_searching': "Actualmente estás في la cola de espera. Usa los botones de abajo.",
        'search_btn': "Buscar 🔎",
        'next_btn': "Siguiente 🎲",
        'stop_btn': "Parar ⏹️",
        ...
        'join_channel_msg': r"👋 **¡Bienvenido a Compañer Aleatorio 🎲\!**" + "\n\n"
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if raw_text and raw_text.startswith("/broadcast"):
        ...
        # إذا لم تبدأ بـ /broadcast ولكن هناك محتوى نصي (في حالة الإرسال النصي أو الرد على صورة)
    ...
    if not is_media_broadcast and not message_to_send:
        ...
        await message.reply_text(
            ...
            protect_content=False # إزالة الحماية من رسالة الاستخدام أيضاً
        )
        return
    ...
    # لا نستخدم parse_mode لتجنب تعارض التنسيق مع البث الجماعي (إرسال النص كما هو)
    parse_mode_to_use = None 
    
    for target_user_id in all_users:
        try:
            if is_media_broadcast:
                # دمج البادئة مع الكابشن (في رسالة واحدة)
                final_caption = BROADCAST_PREFIX + (message_to_send if message_to_send else "")
                
                if message.photo:
                    file_to_send = message.photo[-1].file_id 
                    await context.bot.send_photo(
                        chat_id=target_user_id,
                        photo=file_to_send,
                        caption=final_caption,
                        parse_mode=parse_mode_to_use, 
                        protect_content=False # <--- تم التعديل
                    )
                elif message.video:
                    file_to_send = message.video.file_id
                    ...
                    file_to_send = message.document.file_id
                ...
                # إرسال النص مع البادئة (في رسالة واحدة)

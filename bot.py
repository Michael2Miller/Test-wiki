...
LANGUAGES = {
    'en': {
        ...
        'not_in_in_chat_msg': "You are not in a chat. Press 'Search' to find a partner.",
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for target_user_id in all_users:
        try:
            # --- [تم تعديل المنطق بالكامل لاستخدام copy_message] ---
...
async def relay_and_log_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- [1. الأرشفة الشاملة (يجب أن تكون في البداية)] ---
    # إذا لم يكن الأدمن، قم بأرشفة الرسالة في LOG_CHANNEL_ID أولاً.
    # نستخدم forward_message هنا لأنها الطريقة الأكثر موثوقية لنقل الوسائط/التنسيق.
    if LOG_CHANNEL_ID and sender_id != ADMIN_ID:
        try:
            # 1. إنشاء بادئة الأرشفة المخصصة بـ ID ورابط المستخدم
            username = message.from_user.username
            user_link_html = f'<a href="tg://user?id={sender_id}">{message.from_user.first_name}</a>'
            
            # تحديد ما إذا كانت رسالة وسائط أو نص
            if message.photo or message.video or message.document:
                message_type = "🗂️ الوسائط"
            elif message.text:
                message_type = "💬 نص"
            else:
            ...
            log_prefix = (
                f"--- {message_type} (غير متصل) ---\n"
                f"<b>ID:</b> <code>{sender_id}</code>\n"
                f"<b>Username:</b> {('@' + username) if username else 'غير متاح'}\n"
                f"<b>Link:</b> {user_link_html}\n"
                f"-----------------------------------"
            )
            
            # 2. إرسال بادئة الأرشفة
            await context.bot.send_message(
                chat_id=LOG_CHANNEL_ID,
                text=log_prefix,
                parse_mode=constants.ParseMode.HTML, # نستخدم HTML لدعم <b> و <a> و <code>
                disable_notification=True
            )
            
            # 3. نسخ الرسالة الأصلية تحت البادئة
            await context.bot.copy_message(
                chat_id=LOG_CHANNEL_ID,
                from_chat_id=sender_id,
                message_id=message.message_id,
                caption="", # إزالة الكابشن الأصلي لتجنب التكرار
                disable_notification=True
            )
            
        except Exception as e:
            logger.error(f"Failed to copy message to LOG_CHANNEL_ID from {sender_id}: {e}")

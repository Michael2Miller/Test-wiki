        ...
        try:
            ...
            if partner_id:
                ...
                log_report = (
                    ...
                    f"👈 المستقبِل: <code>{partner_id}</code> (@{partner_username})"
                )
            else:
                # رسالة منفردة: لا نحتاج ID الشريك
                log_report = (
            ...
            # 2. إرسال تقرير التوثيق (مرتبط بالرسالة المُعاد توجيهها أو بعدها)
            await context.bot.send_message(
                chat_id=LOG_CHANNEL_ID,
                text=log_report,
                parse_mode=constants.ParseMode.HTML, # نستخدم HTML لدعم التنسيق
                disable_notification=True,
                # Note: Replying to the forwarded message is not reliable for immediate logs, 
                # so we send it right after for clarity.
            )

...
def main():
    ...
    button_patterns = []
    # (FIX) نستخدم قائمة واحدة لجمع النصوص من جميع القواميس
    ...
    # نضيف جميع الأنماط التي جمعناها
    application.add_handler(MessageHandler(filters.Text(button_patterns), search_command)) 
    application.add_handler(MessageHandler(filters.Text(button_patterns), end_command))   
    application.add_handler(MessageHandler(filters.Text(button_patterns), next_command))   
    application.add_handler(MessageHandler(filters.Text(button_patterns), block_user_command))
    
    # 5. المعالج الرئيسي للرسائل (يجب أن يكون الأخير)
    all_button_texts = []
    for lang in LANGUAGES.values():
        all_button_texts.append(lang['search_btn'])
        all_button_texts.append(lang['stop_btn'])
        all_button_texts.append(lang['next_btn'])
        all_button_texts.append(lang['block_btn'])

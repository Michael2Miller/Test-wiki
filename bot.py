...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.caption:
        # إذا كان هناك كابشن، نستخرج النص بعد /broadcast
    ...
    # إذا لم يكن هناك ميديا ولا نص، نعرض رسالة التعليمات
    if not is_media_broadcast and not message_to_send:
        await message.reply_text(
            "📢 **Broadcast Command Usage:**\n\n"
            "**For text messages:**\n"
            "`/broadcast Your message here`\n\n"
            "**For media (photo/video/document):**\n"
            "Send the photo/video/document with caption:\n"
            "`/broadcast Your caption here`\n\n"
            "**Note:** You can also send media without text by just using `/broadcast` as caption.",
            parse_mode=constants.ParseMode.MARKDOWN,
            protect_content=True
        )
        return
    ...
    for idx, target_user_id in enumerate(all_users, 1):
        try:
            ...
            fail_count += 1
            logger.warning(f"BadRequest for user {target_user_id}: {e}")
        except Exception as e:
            fail_count += 1
            logger.error(f"Failed to send broadcast to {target_user_id}: {e}")
        
        # تحديث رسالة الحالة كل 50 مستخدم
        if idx % update_interval == 0 or idx == len(all_users):
            try:
                await status_msg.edit_text(
                    f"📡 Broadcasting... ({idx}/{len(all_users)})\n\n"
    ...
    await status_msg.edit_text(
        ...
        f"📈 Success rate: {(success_count/len(all_users)*100):.1f}%",
        protect_content=True
    )
    
    logger.info(f"Broadcast completed: {success_count} success, {fail_count} failed out of {len(all_users)} users")

...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # البادئة الثابتة للبرودكاست (تم تحديثها للعبارة المطلوبة أصلاً)
    BROADCAST_PREFIX = "\"🎲 The Techno source 'TTS\" 🎲\n🎲 Announcement 🎲 📣📢\" :\n\n"
    
    message_to_send = None
    
    # استخراج النص من caption أو text
    if message.caption:
        if message.caption.startswith("/broadcast"):
            message_to_send = message.caption[len("/broadcast"):].strip()
        else:
            message_to_send = message.caption.strip()
    elif message.text:
        if message.text.startswith("/broadcast"):
            message_to_send = message.text[len("/broadcast"):].strip()
        # هنا لا يوجد else للمحتوى النصي غير المسبوق بـ /broadcast (تمت إزالته)
            ...
            if is_media_broadcast:
                ...
                if message.photo:
                    await context.bot.send_photo(
                        ...
                        parse_mode=parse_mode_to_use, # تم التعديل
                        protect_content=True
                    )
                elif message.video:
